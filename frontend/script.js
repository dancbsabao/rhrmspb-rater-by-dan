...
.tabs {
  ...
  margin-bottom: 20px;
  position: relative;
  z-index: 1100; /* Above results-area */
}
...
.results-area {
  position: fixed;
  top: 120px; /* Below auth-section and tabs */
  left: 20px;
  right: 20px;
  width: calc(100% - 40px);
  max-width: 1400px;
...
.results-area .close-button:hover {
  color: #000;
}
...
.secretariat-table th.action-column,
.secretariat-table td.action-column {
  min-width: 220px;
  width: 220px;
}
...
.secretariat-table .action-dropdown:focus {
  outline: none;
  border-color: #333;
...
#submitSecretariatActions::before {
  content: '✔';
  position: absolute;
  left: 10px;
  top: 50%;
...
#submitSecretariatActions:hover::before {
  opacity: 1;
}

#submitSecretariatActions:hover {
  background: linear-gradient(to bottom, #444, #111);
  transform: scale(1.05);
  padding-left: 34px;
}

#submitSecretariatActions:active {
...
#submitSecretariatActions:disabled::before {
  opacity: 0;
}
