...
class SimpleAPIManager {
  ...
  async fetchWithCache(key, fetchFunction, options = {}) {
    ...
    try {
      ...
      const staleData = this.getCachedData(key, 30 * 60 * 1000); // 30min tolerance for stale data
      if (staleData) {
        console.log(`🗃️ Fetch failed for ${key}. Using stale cache (age: ${Math.round((Date.now() - staleData.timestamp)/1000)}s)`);
...
// Enhanced wrapper functions for backend API calls
async function safeFetchSecretariatMembers(accessToken) {
  return await apiManager.fetchWithCache('secretariatMembers', async () => {
    const response = await fetch(`${API_BASE_URL}/api/sheets/values?range=SECRETARIAT_MEMBERS!A:D`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      credentials: 'include' // Include cookies for refresh_token
    });
    if (!response.ok) {
      throw new Error(`Failed to fetch secretariat members: ${response.statusText}`);
    }
...
async function safeFetchVacanciesData(accessToken) {
  return await apiManager.fetchWithCache('vacanciesData', async () => {
    const response = await fetch(`${API_BASE_URL}/api/sheets/values?range=SECRETARIAT_VACANCIES!A:C`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      credentials: 'include' // Include cookies for refresh_token
    });
    if (!response.ok) {
      throw new Error(`Failed to fetch vacancies data: ${response.statusText}`);
    }
    return await response.json();
  }, {
...
async function safeLoadSignatories(accessToken) {
  if (!loadSignatories || typeof loadSignatories !== 'function') {
    console.log('loadSignatories function not available');
    return null;
  }
  
  return await apiManager.fetchWithCache('signatories', async () => {
    const response = await fetch(`${API_BASE_URL}/api/sheets/values?range=SECRETARIAT_MEMBERS!E:G`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      credentials: 'include' // Include cookies for refresh_token
    });
    if (!response.ok) {
      throw new Error(`Failed to fetch signatories: ${response.statusText}`);
    }
    return await response.json();
  }, {
      ...
      const apiRequests = [
        {
          ...
          fetchFunction: () => safeFetchSecretariatMembers(accessToken),
          priority: 3,
          required: true
        },
        {
          key: 'vacanciesData',
          fetchFunction: () => safeFetchVacanciesData(accessToken),
          priority: 2,
          required: true
        },
        {
          key: 'signatories',
          fetchFunction: () => safeLoadSignatories(accessToken),
          priority: 1,
          required: false
        }
...
async function handleCriticalAPIFailure(criticalErrors) {
  ...
  for (const error of criticalErrors) {
    const staleData = apiManager.getCachedData(error.key);
    if (staleData) {
      console.log(`🗃️ Using stale cache for critical data: ${error.key}`);
      // Populate UI with stale data if needed
    }
  }
...
setInterval(() => {
  if (apiManager.cache.size > 100) { // Arbitrary limit
